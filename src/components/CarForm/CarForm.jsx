...
// Valores por defecto para el modo "crear" (formulario vacío)
const EMPTY_CAR = {
    name: '',
    type: '',
    image: '',
...
/**
 * Formulario reutilizable de carro.
 * Lo usan CreateCar (sin initialValues → campos vacíos) y
 * EditCar (con initialValues → campos pre-cargados).
 *
 * Props:
 * - title: título del formulario (ej. "Crear nuevo carro")
 * - submitLabel: texto del botón de envío (ej. "Crear carro")
 * - initialValues: valores iniciales del formulario (opcional)
 * - onSubmit: async (carPayload) => void — recibe el objeto con la forma que espera el API
 */
function CarForm({ title, submitLabel, initialValues = EMPTY_CAR, onSubmit }) {
    const [loading, setLoading] = useState(false);

    // Estado del formulario: useForm maneja los valores y los errores por campo
    const { values, handleChange, errors } = useForm(initialValues, validationRules);

    /**
     * Valida TODOS los campos antes de enviar.
     * useForm solo valida los campos que el usuario ha tocado (onChange),
     * así que aquí recorremos todas las reglas para evitar enviar un form inválido.
     */
    const validateAll = () => {
    ...
    return (
        <section className={styles['car-form-section']}>
            <form className={styles['car-form__form']} onSubmit={handleSubmit} noValidate>
                <h2>{title}</h2>
                ...
                <input
                    id="name"
                    name="name"
                    className={styles['car-form__input']}
                    value={values.name}
                    onChange={handleChange}
                    placeholder="Ferrari 488"
                />
                {/* Si el campo tiene error, lo mostramos debajo del input */}
                {errors.name && <span className={styles['car-form__error']}>{errors.name}</span>}
                ...
                {errors.type && <span className={styles['car-form__error']}>{errors.type}</span>}
                ...
                />
                {errors.image && <span className={styles['car-form__error']}>{errors.image}</span>}
                ...
                />
                {errors.speed && <span className={styles['car-form__error']}>{errors.speed}</span>}
                ...
                <input
                    ...
                    placeholder="3.0s"
                />
                {errors.zeroToHundred && <span className={styles['car-form__error']}>{errors.zeroToHundred}</span>}
                ...
                <input
                    ...
                    placeholder="2"
                />
                {errors.seats && <span className={styles['car-form__error']}>{errors.seats}</span>}
                ...
                <input
                    ...
                    placeholder="450"
                />
                {errors.price && <span className={styles['car-form__error']}>{errors.price}</span>}

                {/* Mientras la petición está en curso deshabilitamos el botón */}
